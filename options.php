...
if ($right >= "R") {
	...
	if ($REQUEST_METHOD == "POST" && strlen($Update . $Apply . $RestoreDefaults) > 0 && $right == "W" &&
		check_bitrix_sessid()
	) {
	...
	?>
	<form method="post" name="twigintegrationmodule_opt_form"
				...
				<td width="60%">
					<?php if ($type[0] == "checkbox"): ?>
						<input type="checkbox" name="<?= htmlspecialcharsbx($option[0]) ?>"
							id="<?= htmlspecialcharsbx($option[0]) ?>" value="Y"<?php if ($value == "Y") {
							echo " checked";
						} ?>>
					<?php elseif ($type[0] == "text"): ?>
						<input type="text" size="<?= $type[1] ?>" maxlength="255"
						value="<?= htmlspecialcharsbx($value) ?>" name="<?= htmlspecialcharsbx($option[0]) ?>"
						id="<?= htmlspecialcharsbx($option[0]) ?>"><?php if ($option[0] == "slow_sql_time")
		...
		<tr>
			...
			<td width="50%">
				<input type="button" value="<?= GetMessage('TWIG_INTEGRATION_CLEAR_CACHE_ACTION'); ?>"
					id="clear-twig-cache"/>
		...
		<?php if (strlen($_REQUEST["back_url_settings"]) > 0): ?>
			<input <?php if ($right < "W")
				echo "disabled" ?> type="button" name="Cancel" value="<?= GetMessage("MAIN_OPT_CANCEL") ?>"
				title="<?= GetMessage("MAIN_OPT_CANCEL_TITLE") ?>" onclick="window.location='<?= htmlspecialcharsbx(
